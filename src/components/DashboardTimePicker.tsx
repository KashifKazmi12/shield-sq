...
import { ALERT_WINDOWS, type AlertWindowKey, type AlertWindowPresetKey } from "@/lib/alert-window";
import { formatIsoDate, parseIsoDate } from "@/lib/trend-range";
...
export function DashboardTimePicker() {
  ...
  function applyParams(next: { window: string; from?: string; to?: string }) {
    const params = new URLSearchParams(searchParams.toString());
    params.set("window", next.window);
    if (next.window === "custom" && next.from && next.to) {
  ...
  function selectPreset(key: AlertWindowPresetKey) {
    applyParams({ window: key });
  }
  ...
  function applyCustom() {
    ...
    applyParams({ window: "custom", from: draftStart, to: draftEnd });
  }
        ...
        <div className="trend-range-menu" role="listbox">
          {ALERT_WINDOWS.map((p) => (
            <button
              ...
              className={activeKey === p.key ? "active" : undefined}
          ...
          <button
            type="button"
            role="option"
            aria-selected={activeKey === "custom"}
            className={activeKey === "custom" ? "active" : undefined}
            onClick={openCustom}
          >
            Custom range…
          </button>
