...
import { parseCloudsplainingPayload, extractCloudsplainingFindings, CloudsplainingPayloadError } from "@/lib/cloudsplaining";
import { processBatchScan, ciRunIdempotencyKey, contentHashIdempotencyKey } from "@/lib/batch-ingest";

const TOOL = "cloudsplaining";
...
export async function POST(request: Request) {
  ...
  let parsed;
  try {
    parsed = parseCloudsplainingPayload(body);
  } catch (err) {
    const detail = err instanceof CloudsplainingPayloadError ? err.message : "Invalid Cloudsplaining payload";
  ...
  const result = await processBatchScan({
    ...
    meta: parsed.meta,
    rawPayload: body,
    findings: extractCloudsplainingFindings(parsed.report),
  });
