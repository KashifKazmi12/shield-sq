...
import { parseKubeBenchPayload, extractKubeBenchFindings, KubeBenchPayloadError } from "@/lib/kube-bench";
import { processBatchScan, ciRunIdempotencyKey, contentHashIdempotencyKey } from "@/lib/batch-ingest";

const TOOL = "kube-bench";
...
export async function POST(request: Request) {
  ...
  let parsed;
  try {
    parsed = parseKubeBenchPayload(body);
  } catch (err) {
    const detail = err instanceof KubeBenchPayloadError ? err.message : "Invalid kube-bench payload";
  ...
  const result = await processBatchScan({
    ...
    meta: parsed.meta,
    rawPayload: body,
    findings: extractKubeBenchFindings(parsed.report, parsed.meta.cluster),
  });
