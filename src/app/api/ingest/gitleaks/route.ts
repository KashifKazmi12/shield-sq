...
import { parseGitleaksPayload, extractGitleaksFindings, GitleaksPayloadError } from "@/lib/gitleaks";
...
export async function POST(request: Request) {
  ...
  let parsed;
  try {
    parsed = parseGitleaksPayload(body);
  } catch (err) {
    const detail = err instanceof GitleaksPayloadError ? err.message : "Invalid gitleaks payload";
  ...
  const result = await processBatchScan({
    ...
    meta: parsed.meta,
    rawPayload: body,
    findings: extractGitleaksFindings(parsed.report),
  });
