...
import { parseTrivyLicensePayload, extractTrivyLicenseFindings, TrivyLicensePayloadError } from "@/lib/trivy-license";
import { processBatchScan, ciRunIdempotencyKey, contentHashIdempotencyKey } from "@/lib/batch-ingest";

const TOOL = "trivy-license";
...
export async function POST(request: Request) {
  ...
  let parsed;
  try {
    parsed = parseTrivyLicensePayload(body);
  } catch (err) {
    const detail = err instanceof TrivyLicensePayloadError ? err.message : "Invalid trivy license payload";
  ...
  const result = await processBatchScan({
    ...
    meta: parsed.meta,
    rawPayload: body,
    findings: extractTrivyLicenseFindings(parsed.report),
  });
