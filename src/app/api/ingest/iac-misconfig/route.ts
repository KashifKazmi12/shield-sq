...
import { parseIacMisconfigPayload, extractIacMisconfigFindings, IacMisconfigPayloadError } from "@/lib/iac-misconfig";
import { processBatchScan, ciRunIdempotencyKey, contentHashIdempotencyKey } from "@/lib/batch-ingest";

const TOOL = "iac-misconfig";
...
export async function POST(request: Request) {
  ...
  let parsed;
  try {
    parsed = parseIacMisconfigPayload(body);
  } catch (err) {
    const detail = err instanceof IacMisconfigPayloadError ? err.message : "Invalid IaC misconfig payload";
  ...
  const result = await processBatchScan({
    ...
    meta: parsed.meta,
    rawPayload: body,
    findings: extractIacMisconfigFindings(parsed.report),
  });
